...
`BlockPoller#Selector`是在`NioBlockingSelector#open()`方法中传入的`NioSelectorPool#Selector SHARED_SELECTOR`。因为它是个线程，所以下面分析`run()`方法。`run()`方法中的while()循环中：
