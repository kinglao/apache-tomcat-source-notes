...
"main@1" prio=5 tid=0x1 nid=NA runnable
  java.lang.Thread.State: RUNNABLE
	  ...
	  at org.apache.catalina.startup.Bootstrap.main(Bootstrap.java:495)
```
在`HostConfig#lifecycleEvent Lifecycle.START_EVENT`-->`HostConfig#start()`-->`deployApps()`中：
...
* 对每一个ContextName，调用`HostConfig#deployDirectory()`方法，将这五个Context封装为`StandardContext`
