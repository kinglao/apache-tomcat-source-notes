...
15. `CoyoteAdapter#service()`方法中，调用了`connector.getService().getContainer().getPipeline()`. `connector`-->`Connector`，`connector.getService()`-->`StandardService`，`connector.getService().getContainer()`-->`StandardEngine`，`connector.getService().getContainer().getPipeline()`-->`StandardPipeline`。
