...
> `Pipeline`使用了责任链模式，`Pipeline`里面存有一些`Valve`，这些`Valve`按照加入的顺序执行，可以选择忽略并交给下一个`Value`，或者处理`Request`并给出`Response`返回，但是不能所有的`Value`都选择忽略，所以最后一个`Valve`总是会进行处理。`setBasic()`方法就是设置了那个最后的`Value`。每一个容器（`ContainerBase`的子类）都有一个`Pipeline`域，在他们各自的的构造方法中调用`Pipeline#setBasic()`方法设置basic value：`StandardContext`-->`StandardContextValue`，`StandardEngine`-->`StandardEngineValue`，`StandardHost`-->`StandardHostValue`，`StandardWrapper`-->`StandardWrapperValue`
...
* `AbstractProcessor#action()`. 根据不同的`ActionCode`做不同的处理。
