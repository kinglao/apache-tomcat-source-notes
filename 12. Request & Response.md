...
> `Pipeline`使用了责任链模式，`Pipeline`里面存有一些`Valve`，这些`Value`按照加入的顺序执行，可以选择忽略并交给下一个`Value`，或者处理`Request`并给出`Response`返回，但是不能所有的`Value`都选择忽略，所以最后一个`Value`总是会进行处理。`setBasic()`方法就是设置了那个最后的`Value`。每一个容器（`ContainerBase`的子类）都有一个`Pipeline`域，在他们各自的的构造方法中调用`Pipeline#setBasic()`方法设置basic value：`StandardContext`-->`StandardContextValue`，`StandardEngine`-->`StandardEngineValue`，`StandardHost`-->`StandardHostValue`，`StandardWrapper`-->`StandardWrapperValue`

这里再回到上面</br>
* 调用`connector.getService().getContainer().getPipeline().getFirst().invoke(request, response);`，因为`StandardEngine`的pipeline里面只有一个value：`StandardEngineValue`，所以调用`StandardEngineValue#invoke()`方法，最后调用的是`host.getPipeline().getFirst().invoke(request, response)`。
* `host`值是`StandardHost`，然后`StandardHostValue`的`invoke()`方法中，调用了`context.getPipeline().getFirst().invoke(request, response);`。
* `context`对应`StandardContext`，`StandardContextValue`的`invoke()`方法中，会调用`wrapper.getPipeline().getFirst().invoke(request, response);`. 
* `wrapper`对应`StandardWrapper`，调用`StandardWrapperValue`的`invoke()`方法。

当然期间有很多分支，不一定就一直能走到`StandardWrapperValue`的`invoke()`方法. 如果有其他分支，一般的调用顺序是：</br>
