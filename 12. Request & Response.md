...
9. `CoyoteAdapter#postParseRequest()`方法中，又会调用`connector.getService().getMapper().map(serverName, decodedURI, version, request.getMappingData());`。对应的容器分别是：`Connector`、`StandardService`。
