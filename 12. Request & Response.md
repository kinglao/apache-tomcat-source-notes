...
```
11. 在Tomcat启动过程中（start），调用了`Mapper$ContextList.addContext()`方法，将`MappedContext`加入到`Mapper$ContextList#MappedContext[] contexts`域中。用来保存所有的context path
```
"main@1" prio=5 tid=0x1 nid=NA runnable
  java.lang.Thread.State: RUNNABLE
	  ...
	  at org.apache.catalina.startup.Bootstrap.main(Bootstrap.java:495)
```
在`MapperListener.registerHost()`中，遍历`StandardHost`中的child，调用`registerContext()`
```java
...
12. 再看一下`StandardHost`中的child是怎么加入的。在`HostConfig#lifecycleEvent Lifecycle.START_EVENT`-->`HostConfig#start()`-->`deployApps()`中：
...
13. `Mapper#map()`方法中，调用了`Mapper#internalMap()`方法，
...
* Context mapping. 匹配路径，采用最长路径匹配法。调用`Mapper.find()`方法，传入的`MappedContext[]`是已经排好序的数组（插入的时候排的序，调用`Mapper#insertMap()`插入，调用`Mapper#find()`寻找插入位置）
```
...
14. `CoyoteAdapter#service()`方法中，调用了`connector.getService().getContainer().getPipeline()`. `connector`-->`Connector`，`connector.getService()`-->`StandardService`，`connector.getService().getContainer()`-->`StandardEngine`，`connector.getService().getContainer().getPipeline()`-->`StandardPipeline`。
...
15. `CoyoteAdapter#service()`方法中，调用完`connector.getService().getContainer().getPipeline().getFirst().invoke(request, response);`后，在finally中，会调用
```java
