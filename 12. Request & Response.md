...
```
查找过程使用二分查找，返回数组下标。找到对应的`MappedContext`后，设置`MappingData`的属性值：`contextPath`、`contexts`、`context`、`contextSlashCount`
* Wrapper mapping. 调用`Mapper#internalMapWrapper()方法`。上一步找到了Context，也就是路径，这一步匹配对应的Servlet。</br>
Rule 1 -- Exact Match</br>
Rule 2 -- Prefix Match</br>
Rule 3 -- Extension Match</br>
Rule 4 -- Welcome resources processing for servlets</br>
Rule 4a -- Welcome resources processing for exact macth</br>
Rule 4b -- Welcome resources processing for prefix match</br>
Rule 4c -- Welcome resources processing for physical folder</br>
Rule 7 -- Default servlet</br>
