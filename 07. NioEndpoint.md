...
为`NioEndpoint`的`stopLatch`域设置值：`CountDownLatch stopLatch = new CountDownLatch(pollerThreadCount)`。初始数量是`Poller`线程的数量，默认大小的定义是：`private int pollerThreadCount = Math.min(2,Runtime.getRuntime().availableProcessors());`，最大是2个。`Poller`线程在`NioEndpoint#startInternal()`方法中创建并启动</br>
