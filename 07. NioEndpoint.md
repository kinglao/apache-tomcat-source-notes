...
3.2. `Poller#processKey()`方法中，判断`SelectionKey`是可读的还是可写的，调用`AbstractEndpoint#processSocket()`方法，来处理`NioSocketWrapper`attachment</br>
3.3. `AbstractEndpoint#processSocket()`方法中，将`NioSocketWrapper`封装为`NioEndpoint#SocketProcessor`（`Nio2Endpoint#SocketProcessor`），然后调用`SocketProcessor`的run()方法</br>
...
在`NioEndpoint#Poller#run()`或者`NioBlockingSelector.BlockPoller#run()`方法中调用`Selector#select()`方法或者`Selector#selectNow()`方法，然后遍历每一个`selector.selectedKeys()`，进行处理。
