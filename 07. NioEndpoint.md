...
1. 拿到`Processor`对象. 第一次调用的时候应该是通过下面的代码拿到的
```java
if (processor == null) {
...
```
2. 拿到processor对象后，在do...while循环中调用`state = processor.process(wrapper, status);`方法进行处理，实际上调用的是`AbstractProcessorLight#process()`方法，在这里面处理Http，用到了`AsyncStateMachine`状态机。
