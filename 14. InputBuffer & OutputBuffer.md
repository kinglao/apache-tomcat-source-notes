...
2. 如果from buffer的remaining（设为FR）大于to的remaining（设为TR），则max是TR，则从from buffer中将TR容量的数据写入到to buffer中，from buffer中还有 FR-TR 的数据没读取。

3. 如果from buffer的remaining FR小于to buffer的remaining TR，则max是FR，则从from buffer中将FR容量的数据写入到to中，from buffer中已经无数据可读

`NioEndpoint$NioSocketWrapper.read()`方法：
1. `SocketWrapperBase.populateReadBuffer()`的作用是将from buffer中的数据转移到to buffer中。实际调用的是上面的`transfer()`方法，如果`transfer()`方法不返回0，则`read()`方法结束。走到下面的步骤说明至少其中一个buffer的position等于limit，即from buffer中已经无数据可读或者to buffer中已经满了，to buffer无法写入更多的数据。
